...
func NewClaim(data jcommon.Bytes) *Claim {
	return &Claim{
		Iat:  time.Now().Unix(),
...
type Claim struct {
	...
	Data jcommon.Bytes `json:"data,omitempty"`
}
...
func (cl Claim) Valid() error {
	if time.Now().Unix() < cl.Nbf {
		return jcommon.NewErr(401, "還沒到生效時間再等等吧")
	}
	if cl.Exp > 0 && cl.Exp < time.Now().Unix() {
		return jcommon.NewErr(401, "過期了重新要一個吧")
	}
	if cl.Iss != "asgame" {
		return jcommon.NewErr(401, "這不是我發的,別亂喔")
	}
	// data裡面的內容個應用程式自己檢查
	return nil
}
